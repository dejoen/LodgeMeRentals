...
const AgentDashBoardScreen = () => {
  ...
  const { socketConnectedReducerState, connectSocket, agentReducerDispatcher, notificationState } =
    useManageAgentState();

  useEffect(() => {
    if (!socketConnectedReducerState.isSocketConnect && agentState.isLoggedIn) {
  ...
  useState(() => {
    getNotitifications(agentState.data.token)
      .then((rs) => {
        return rs.json();
      })
      .then((result) => {
    ...
    return (
      <div className="flex h-[100vh] w-full overflow-x-hidden">
        ...
        {!agentState.data.isAgentFileAlreadyUploaded && (
          <AgentVerificationPopUp showScreen={true} token={agentState.data.token} />
        )}
      </div>
    );
  } else {
